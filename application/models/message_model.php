...
class Message_model extends CI_Model {
	 ...
	 *@param array 留言信息数组
	 */
	function get_message($message_id, $fields='*')
	{
		$query = $this->db->select($fields)->get_where($this->message_table, array('message_id' => $message_id), 1, 0);
	 ...
	 *@return 留言信息集
	 */
	function get_messages($limit = NULL, $offset = 0, $fields='*')
	{
		$this->db->select('messages.*, COUNT(comments.message_id) as comment_num')->from('messages')
			->join('comments', 'messages.message_id=comments.message_id', 'left')->group_by('messages.message_id');
	 ...
	 *@return 评论信息集
	 */
	function get_comments($message_id, $limit = NULL, $offset = 0)
	{
